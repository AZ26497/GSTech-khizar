...
import { signInRequets, signInResponse, signInError } from '../redux/actions/signInActions'
...
const SignIn = ({ navigation, route }) => {
    ...
    mobileNumberValidate = (text) => {
        const reg = /^((\+92)|(0092))-{0,1}\d{3}-{0,1}\d{7}$|^\d{11}$|^\d{4}-\d{7}$/;////^0|08[0-9]{9,}$/;///^[0]?[789]\d{9}$/;
            ...
            >
                ...
                <View style={styles.container}>
                    <View style={styles.textFieldCont}>
                        ...
                        />
                        {phoneNumErrorMsg != '' && <Text style={{ color: 'red', fontSize: 16, textAlign: 'right' }}>{phoneNumErrorMsg}</Text>}
                    ...
                    {loading == true ?
                        <Loader />
                        :
                        <GradientButton height={60} width={'90%'} title={'Sign In'} margin={5} action={() => loginApiCall()} />
                    }
                </View>
            </LinearGradient>
        </SafeAreaView>
...
export default connect(mapStateToProps, { signInRequets, signInResponse, signInError })(SignIn)
